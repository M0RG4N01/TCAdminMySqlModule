...
namespace MySqlModule.Controllers
{
    ...
    public class MySqlController : BaseController
    {
        ...
        public ActionResult CreateDatabase(int requestServiceId1, string requestDbName)
        {
            ...
            {
                return JavaScript("TCAdmin.Ajax.ShowBasicDialog('Error', 'You have reached your database limit!');$('body').css('cursor', 'default');");
            }
            if (requestDbName == string.Empty)
            {
                return JavaScript("TCAdmin.Ajax.ShowBasicDialog('Error', 'You did not enter a database name into the textbox!');$('body').css('cursor', 'default');");
            }
            if (!Regex.IsMatch(requestDbName, "^[_a-zA-Z0-9 ]*$"))
            {
                return JavaScript("TCAdmin.Ajax.ShowBasicDialog('Error', 'That database name is not allowed due to illegal characters!');$('body').css('cursor', 'default');");
            }
            ...
            if (service == null)
            {
                return JavaScript("TCAdmin.Ajax.ShowBasicDialog('Error', 'You don't own this service');$('body').css('cursor', 'default');");
            }
            ...
            if (server.MySqlPluginUseDatacenter && datacenter.MySqlPluginIp != string.Empty)
            {
                ...
                {
                    return JavaScript("TCAdmin.Ajax.ShowBasicDialog('Error', 'Unable to connect to the remote MySQL host. Please contact an Administrator!');$('body').css('cursor', 'default');");
                }
            ...
            else if(server.MySqlPluginUseDatacenter == false && server.MySqlPluginIp != string.Empty)
            {
                try{
                    var createDb = new MySqlConnection("server=" + server.MySqlPluginIp + ";user=" +
                                                   server.MySqlPluginRoot + ";password=" + server.MySqlPluginPassword +
                                                   ";");
                    createDb.Open();
                    var cmd = createDb.CreateCommand();
                    var createDbSql = string.Concat(
            ...
            {
                return JavaScript("TCAdmin.Ajax.ShowBasicDialog('Error', 'An Administrator has not configured the location of this service for the MySql Module!');$('body').css('cursor', 'default');");
            }
            service.Save();
        ...
        public ActionResult DeleteDatabase(int requestServiceId2)
        {
            ...
            {
                try{
                    var deleteDb = new MySqlConnection("server=" + datacenter.MySqlPluginIp + ";user=" + datacenter.MySqlPluginRoot + ";password=" + datacenter.MySqlPluginPassword + ";");
                    deleteDb.Open();
            ...
            {
                try{
                    var deleteDb = new MySqlConnection("server=" + server.MySqlPluginIp + ";user=" + server.MySqlPluginRoot + ";password=" + server.MySqlPluginPassword + ";");
                    deleteDb.Open();
                    var cmd = deleteDb.CreateCommand();
                    var deleteDbSql = string.Concat(
                        "DROP USER " + dbUser + "@`%`;"
        ...
        public ActionResult ResetPassword(int requestServiceId3)
        {
            ...
            {
                try{
                    var resetDb = new MySqlConnection("server=" + datacenter.MySqlPluginIp + ";user=" + datacenter.MySqlPluginRoot + ";password=" + datacenter.MySqlPluginPassword + ";");
                    resetDb.Open();
            ...
            {
                try{
                    var resetDb = new MySqlConnection("server=" + server.MySqlPluginIp + ";user=" + server.MySqlPluginRoot + ";password=" + server.MySqlPluginPassword + ";");
                    resetDb.Open();
                    var cmd = resetDb.CreateCommand();
                    var resetDbSql = "SET PASSWORD FOR '" + dbUser + "' = PASSWORD('" + dbPass + "');";
                    cmd.CommandText = resetDbSql;
        ...
        private static List<SelectListItem> GetDbDeletionUsernames()
        {
            ...
            return (from service in services let usernameExists = service.Variables["_MySQLPlugin::Username"] != null where usernameExists let text = service.Variables["_MySQLPlugin::Database"].ToString() select new SelectListItem {Text = text, Value = service.ServiceId.ToString()}).ToList();
        }
        ...
        private static List<SelectListItem> GetUserServices()
        {
            ...
            return (from service in services let usernameExists = service.Variables["_MySQLPlugin::Username"] != null where !usernameExists select new SelectListItem {Text = service.ConnectionInfo + " - " + service.Name, Value = service.ServiceId.ToString()}).ToList();
        }
        private static MySqlEligibleLocations GetLocations()
        {
            ...
            foreach (var datacenter in services.Select(service => new Datacenter(new Server(service.ServerId).DatacenterId)).Where(datacenter => !datacenters.Any(x => x.DatacenterId == datacenter.DatacenterId)))
            {
